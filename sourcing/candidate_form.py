"""
candidate_form.py  -  Member 1 | Day 3
Zero custom imports. Everything is inside this one file.
...
# Load .env file - works from any folder
...
if not jobs:
    st.warning("No open positions right now. Check back soon.")
    st.stop()

job_options = {
    f"{j['job_title']} at {j['company_name']}": j["id"]
    for j in jobs
}
        ...
        with st.spinner("Submitting..."):
            try:
                ...
                <div style='background:#0d2b1a; border:1px solid #166534;
                  ...
                  <div style='color:#7878a0; font-size:0.85rem; margin-top:0.3rem;'>
                    {selected_label}
                  </div>
                </div>
                """, unsafe_allow_html=True)
